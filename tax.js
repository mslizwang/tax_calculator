...
function calculator(month, income, bonus, tax_free_income, security_n_fund, tax_threshold, special_deduction, other_deduction) {
  //根据月份，确定每月的累计应税额
    ...
    while (i < month){
      i+=1;
      var current_taxable_income = income - tax_free_income - security_n_fund - tax_threshold - special_deduction - other_deduction;
      current_taxable_income = current_taxable_income * i;
      current_taxable_income = parseFloat(current_taxable_income);
    ...
    for (item in taxable_income_by_month){
      var current_rate = r_n_d(item)[1];
      rate_by_month.push(current_rate);
      var current_deduction = r_n_d(item)[2];
      deduction_by_month.push(current_deduction);
      var total_tax_item = item * current_rate - current_deduction;
      total_tax_item = parseFloat(total_tax_item);
    ...
    if (bonus > 0){
      ...
      total_tax[-1] = taxable_income_by_month[taxable_income_by_month.length-1] * current_rate;
      current_tax = total_tax[total_tax.length-1] - paid_tax;
    }
    else {
      flag = 0;
